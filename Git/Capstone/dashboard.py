...
def get_statistic(current_user, token,id):
    ...
    cursor.execute("select count(*) as total_zone, round(avg(z.usable), 2) as average_space, round(avg(z.honeycomb_rate),2) as average_rate from zone_statistics z, refreshed_zones r where z.zone_id = r.zone_id and r.project_id = %s;",[id])
