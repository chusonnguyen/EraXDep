  ...
  if (indicator < wsaaLoop or (wsaaLoop == 0 and indicator == 0)) :
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        for algo in pack_algo_list:
            bin_results = [executor.submit(sorting, width, length , algo, heuris, remainItemList, sum(crateItem.area for crateItem in crate_instances_use), 'ASCA') for heuris in heuristicList]
    ...
    if wsaaLoop != 0:
        useAllCrateList.sort(key= lambda c: (c.honeycomb,c.spaceUsed), reverse=True)
        unuseAllCrateList.sort(key= lambda c: (c.honeycomb,c.spaceUsed), reverse=True)
    else:
...
def mainRow(df, w, l ,totalPoll, pollRow, pollW, pollL, pollX, pollY, pollGap, pollRGap):
    ...
    if wsaaLoop != 0:
        algorythmExecute(maximal, binWidth, pollPosition + pollLength - 0.25, crate_instances_use, crate_instances)
    else:
